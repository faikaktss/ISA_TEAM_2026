    ...
    def read_joystick(self):
        try:
            if self.arduino is None:
                # Simülasyon: sabit test değerleri
                dumen  = 127
                ileri  = 50
                vites  = 0
                otonom = 0
                ...
                parts = data.split(',')
                if len(parts) != 4:
                    print(f'Yanlış format (4 değer bekleniyor): {data}')
                    return
            ...
            mode_str = "MANUEL" if otonom == 0 else "OTONOM"
            print(f'Sag-Sol: {dumen:4d} | Ileri-Geri: {ileri:4d} | '
                  f'Vites: {vites} | Mod: {mode_str}')
